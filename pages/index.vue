<script setup>
definePageMeta({
  layout: "main",
});
useHead({
  title: "Home",
});
</script>
<template>
  <section class="content">
    <div class="container mx-auto">
      <div class="prose">
        <h1 class="text-center">Hey There 👋</h1>
        <p>
          I am MageshBabu, a Software Developer working in
          <a href="https://fossfreks.in">FossFreaks</a>
        </p>
        <p>
          I work on different kinds of projects and various tech stack. Created
          projects, most of them are private for now. New projects are yet to
          come. I kinda like simple but yet aesthetic design so this is why I
          choose this design for this.
        </p>
        <p>
          Also planning to create a blog with this site. soon will be updating
          this site. You can always say hello by using one of
          <nuxt-link to="contact">these</nuxt-link>
        </p>
        <p>
          Other than programming, I also enjoy listening to music, Spending time
          with Loved ones and my pets. I'm a person who seeks bit of privacy,
          which is why I'm not sharing all my details here. you can contact me
          via my handles. Thank you for Spending time reading this, Have a nice
          day.
        </p>
        <hr />
      </div>
    </div>
  </section>
</template>
